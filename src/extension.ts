...
class CustomHoverProvider implements vscode.HoverProvider {
    provideHover(document: vscode.TextDocument, position: vscode.Position): vscode.ProviderResult<vscode.Hover> {
        ...
        if (undefMsg) {
            ...
            const md = new vscode.MarkdownString(
                `🔴 **Variable '${varName}' is not defined**\n\n` +
                `📝 ${MESSAGES.undefined.en(varName)}\n` +
                `🇮🇳 ${MESSAGES.undefined.hi(varName)}`
            );
            md.isTrusted = true;
            return new vscode.Hover(md);
        }
...
function updateDiagnostics(document: vscode.TextDocument, collection: vscode.DiagnosticCollection) {
    ...
    // 2️⃣ Missing semicolons (JS/TS)
    detectMissingSemicolons(lines, diagnostics, lang);

    // 3️⃣ Undefined variables
    detectUndefinedVariables(text, document, diagnostics, lang);
...
function getAllDeclaredVariables(text: string): Set<string> {
    ...
    const funcDeclRegex = /function\s+(\w+)\s*\(/g;
    while ((match = funcDeclRegex.exec(text))) vars.add(match[1]);

    // Function parameters
    const funcParamsRegex = /function\s+\w*\s*\(([^)]*)\)/g;
    while ((match = funcParamsRegex.exec(text))) {
        match[1].split(',').map(p => p.trim()).forEach(p => { if (p) vars.add(p); });
    }
